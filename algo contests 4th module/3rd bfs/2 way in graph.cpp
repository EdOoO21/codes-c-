...
int main() {
    ...
    if (input.is_open()) {
        ...
        std::vector<std::vector<int64_t>> answer(N + 1,  {((N*(N-1))/2) + 1, -2});
        answer[start] = {0, -3};
        for (int j = 0; j < N + 1; j++) {
            for (auto y : distance[j]) {
                for (int i = 1; i < N + 1; ++i) {
                    if ((graph[y][i] == 1) && (y != i) && (answer[i][0] > j + 1)) {
                        answer[i][0] = j + 1;
                        answer[i][1] = y;
        ...
        if (answer[end][1] == -2) {
            ...
            std::cout << answer[end][0] << std::endl;
            int j = end;
            std::vector<int> ans{j};
            while (answer[j][1] != -3) {
                ans.push_back(answer[j][1]);
                j = answer[j][1];
            }
            if (start != end) {
